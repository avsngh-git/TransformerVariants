...
\subsection{Inference and long-context gap fill}

A separate representative-checkpoint benchmark measures greedy generation with a
...
returns a reusable prompt cache.  Persistent cache memory counts the tensor
storages actually retained by that opaque cache.  Long-context perplexity uses
one contiguous held-out validation sample at 1024, 2048, and 4096 tokens.  These
numbers are diagnostics, not seed-aggregated quality estimates.  The raw benchmark
and HTML dashboard additionally retain target-free prefill throughput for every
supported context length.  Validation loss is timed separately so cross-entropy
work is not mislabeled as inference prefill.
...
\begin{tabular}{lrrrr}
\toprule
Recipe & Uncached tok/s & Cached tok/s & PPL@1K & PPL@4K \\
\midrule
Vanilla & 477.2 & 516.2 & 23.8 & unsupported \\
Modern & 326.2 & 329.8 & 21.7 & 146.0 \\
ALiBi & 374.1 & unsupported & 28.8 & 34.7 \\
GQA & 267.5 & unsupported & 31.1 & 143.8 \\
SWA & 264.3 & unsupported & 30.6 & 34.7 \\
SWA-interleaved & 264.4 & unsupported & 31.1 & 44.6 \\
Linear & 94.0 & unsupported & 27.3 & 287.3 \\
MoE & 45.5 & 89.7 & 18.6 & 142.8 \\
MoE-interleaved & 80.8 & 139.4 & 20.2 & 137.3 \\
MoE-deep & 81.3 & 138.3 & 19.9 & 147.0 \\
\bottomrule
\end{tabular}
\caption{Representative-checkpoint inference and long-context diagnostics.
\emph{Unsupported} is an explicit capability result, not a zero or failed timing.}
\label{tab:inference-long-context-final}
\end{table}

Vanilla cannot exceed its learned 1024-position table without defining a new
position-extension method.  ALiBi shows the smallest degradation in this
one-sample extrapolation test.  The corrected causal linear model runs at longer
contexts, but its perplexity deteriorates sharply; linear asymptotic complexity
does not imply length generalization.  ALiBi, GQA, and both SWA recipes currently
return no reusable cache after a multi-token FlashAttention prefill, and the
causal linear recipe does not yet expose its recurrent prefix state through the
shared cache interface.  Their cached generation rows are therefore marked
unsupported rather than benchmarked through a semantically incorrect fallback.
