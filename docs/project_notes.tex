...
\subsection{Mixture of Experts: conditional computation via sparse routing}
\label{sec:moe}

\subsubsection{Baseline role: single dense FFN}

In V0 and V1, every token passes through the same FFN in each layer. The FFN's total parameter count equals its per-token active parameter count: all weights participate in processing every token.

\subsubsection{Limitation and design pressure}

Scaling model capacity by widening the FFN is expensive: doubling $d_h$ doubles per-token FLOPs. The constraint is: increase representational capacity (total parameters) without proportionally increasing per-token compute (active parameters). This is the conditional computation hypothesis: not all tokens need the same features, so different subsets of parameters can specialize.

\subsubsection{Replacement mechanism: top-$k$ expert routing}

A Mixture of Experts (MoE) layer replaces the single dense FFN with $N$ parallel expert FFNs and a learned router. Each token is processed by only the top-$k$ experts (with $k \ll N$):

\begin{definition}[MoE Feed-Forward Layer]
Given input $\vx \in \reals^{\dmodel}$, define:
\begin{align}
    \text{Router scores: } & \bm{p} = \softmax(\mW_r \vx), \quad \mW_r \in \reals^{N \times \dmodel}, \label{eq:moe_router} \\
    \text{Expert selection: } & \mathcal{T} = \mathrm{top\text{-}k}(\bm{p}), \quad |\mathcal{T}| = k, \label{eq:moe_topk} \\
    \text{Renormalized weights: } & \tilde{p}_i = \frac{p_i}{\sum_{j \in \mathcal{T}} p_j}, \quad i \in \mathcal{T}, \label{eq:moe_renorm} \\
    \text{MoE output: } & \mathrm{MoE}(\vx) = \sum_{i \in \mathcal{T}} \tilde{p}_i \cdot \mathrm{Expert}_i(\vx). \label{eq:moe_output}
\end{align}
Each $\mathrm{Expert}_i$ is an independent SwiGLU FFN with its own parameters $(\mW^{(i)}_{\mathrm{gate}}, \mW^{(i)}_{\mathrm{up}}, \mW^{(i)}_{\mathrm{down}})$.
\end{definition}

In this project (V6), $N=8$ experts with $k=2$ (Mixtral-style). The attention, normalization, embeddings, and output head remain identical to V1.

\subsubsection{Load-balancing auxiliary loss}

Without regularization, the router can collapse to sending all tokens to one or two experts (expert collapse). A load-balancing loss penalizes uneven routing:
\begin{equation}\label{eq:aux_loss}
    \loss_{\mathrm{aux}} = \alpha \cdot N \cdot \sum_{i=1}^{N} f_i \cdot P_i,
\end{equation}
where $f_i$ is the fraction of tokens routed to expert $i$ in a batch and $P_i = \frac{1}{BT}\sum_{b,t} p_{b,t,i}$ is the mean router probability for expert $i$. When routing is uniform, $f_i = P_i = 1/N$, and the loss equals $\alpha$.

A z-loss stabilizes router logit magnitudes:
\begin{equation}\label{eq:z_loss}
    \loss_z = \beta \cdot \frac{1}{BT}\sum_{b,t}\left(\log\sum_{i=1}^{N}\exp(z_{b,t,i})\right)^2,
\end{equation}
where $z_{b,t,i}$ are the raw router logits before softmax. The total training loss is $\loss_{\mathrm{CE}} + \loss_{\mathrm{aux}} + \loss_z$.

\begin{intuitionbox}[Why the product $f_i \cdot P_i$]
The product couples two quantities: $f_i$ reflects where tokens actually went (a non-differentiable hard assignment), and $P_i$ reflects what the router \emph{wanted} to do (a differentiable soft probability). The gradient of $\loss_{\mathrm{aux}}$ flows through $P_i$, nudging the router to spread probability mass toward underused experts. The multiplicative form penalizes the joint event ``expert $i$ already gets many tokens \emph{and} the router keeps assigning high probability to it.''
\end{intuitionbox}

\subsubsection{Active versus total parameters}

\begin{definition}[Active Parameters]
The parameters that participate in processing a single token:
\begin{equation}
    \text{Active} = \underbrace{\text{embed} + \text{attention} + \text{norms} + \text{head}}_{\text{shared}} + k \cdot (\text{params per expert}) \cdot L_{\mathrm{MoE}} + (\text{dense FFN params}) \cdot L_{\mathrm{dense}},
\end{equation}
where $L_{\mathrm{MoE}}$ is the number of MoE layers and $L_{\mathrm{dense}}$ is the number of dense layers.
\end{definition}

For fair comparison with V1, V6 matches active parameters ($\approx 51$M at main scale). Total parameters are $\approx 170$M because all $N$ experts are materialized. The ratio $\text{Total}/\text{Active} \approx N/k$ when the FFN dominates parameter count.

\subsubsection{Sub-variants: where to place MoE layers}

Three layer placement strategies explore different MoE density:
\begin{itemize}[nosep]
    \item \textbf{V6-full}: all $\nlayer$ layers use MoE. Total params $\approx N/k$ times V1.
    \item \textbf{V6-interleaved}: odd-indexed layers use MoE, even-indexed layers use dense SwiGLU. Half the expert parameters of V6-full.
    \item \textbf{V6-deep}: layers with index $\geq \nlayer/2$ use MoE, earlier layers use dense. Tests the hypothesis that early layers learn general representations while deeper layers benefit from specialization.
\end{itemize}

\subsubsection{New trade-off}

\begin{tradeoffbox}[MoE costs]
\begin{enumerate}[nosep]
    \item \textbf{Memory:} all $N$ expert weights reside in memory even though only $k$ are active per token.
    \item \textbf{Load imbalance:} requires auxiliary loss tuning ($\alpha, \beta$). Too much regularization degrades quality; too little leads to expert collapse.
    \item \textbf{Expert redundancy risk:} with limited data, experts may converge to similar functions, negating the capacity benefit.
    \item \textbf{Non-uniform computation:} different tokens activate different parameters, complicating FLOP accounting and hardware utilization analysis.
\end{enumerate}
\end{tradeoffbox}

\begin{motivationbox}[Why MoE is in the project]
MoE demonstrates a fundamentally different scaling strategy: increase capacity without increasing per-token compute. The comparison answers: ``For a fixed per-token FLOP budget, does conditional expert routing improve next-token prediction over a single dense FFN?'' This is orthogonal to the attention-mechanism comparisons (V2--V5).
\end{motivationbox}
...
\item \textbf{Sliding Window Attention / SWA (V4)} \emph{(home: \cref{sec:attention-position})}

\begin{itemize}[nosep]
    \item \textbf{Baseline role:} Full causal attention allows every query at position $i$ to attend to all positions $j \leq i$. This gives maximum expressiveness — any pattern of information retrieval is representable within the attention weight simplex.

    \item \textbf{Limitation:} Full causal attention has two practical limitations as sequence length grows:
    \begin{enumerate}[nosep]
        \item Even with FlashAttention's tiling, the $O(T^2)$ arithmetic cost applies to every layer at every training step. For long sequences, much of this compute goes to attending to very distant tokens whose relevance may be small.
        \item The KV-cache at inference scales as $O(\nlayer \cdot T \cdot \dmodel)$. In serving scenarios with long contexts, this dominates memory.
    \end{enumerate}
    Additionally, empirical evidence suggests that lower Transformer layers primarily capture local patterns (syntax, short phrases), while only upper layers benefit from long-range attention. Applying full attention uniformly at every layer may be over-parameterizing the attention pattern for lower layers.

    \item \textbf{Design pressure:} Reduce training FLOPs for attention while preserving the model's ability to capture local patterns that dominate natural language structure. The hypothesis: restricting attention to a fixed local window during training is a useful inductive bias that encourages the model to encode information into the residual stream efficiently, rather than relying on attending to arbitrarily distant raw tokens.

    \item \textbf{Replacement mechanism --- windowed causal attention:} Each query at position $i$ can attend only to positions in the window $[\max(0, i - W + 1), \; i]$, where $W$ is the window size. The causal mask is replaced by a banded causal mask:
    \begin{equation}\label{eq:swa_mask}
        A_{ij} = \begin{cases}
            \vq_i^\top \vk_j / \sqrt{\dhead}, & \max(0, i - W + 1) \leq j \leq i,\\
            -\infty, & \text{otherwise}.
        \end{cases}
    \end{equation}
    After softmax, positions outside the window receive zero weight.

    \item \textbf{Mathematical formulation:} The windowed attention output at position $i$ is:
    \begin{equation}
        \vo_i = \sum_{j=\max(0,\, i-W+1)}^{i} \alpha_{ij} \vv_j, \qquad
        \alpha_{ij} = \frac{\exp(\vq_i^\top \vk_j / \sqrt{\dhead})}{\sum_{k=\max(0,\, i-W+1)}^{i} \exp(\vq_i^\top \vk_k / \sqrt{\dhead})}
    \end{equation}
    The normalisation denominator sums only over the window, not all past positions. Thus the attention weights $\alpha_{ij}$ still form a valid probability distribution (non-negative, sum to 1) over the restricted set.

    \item \textbf{Window size as a fixed proportion:} In this project, $W = \lfloor T / 4 \rfloor$ where $T$ is the training sequence length:
    \begin{center}
    \begin{tabular}{lcccc}
    \toprule
    Scale & $T$ (seq\_len) & $W$ (window) & Ratio $W/T$ & Visible positions at $i = T$\\
    \midrule
    debug & 512 & 128 & 1/4 & 128\\
    main & 1024 & 256 & 1/4 & 256\\
    stretch & 1024 & 256 & 1/4 & 256\\
    \bottomrule
    \end{tabular}
    \end{center}
    A fixed proportion rather than a fixed absolute window means the receptive field scales with context length. Each layer sees a local window; stacking $\nlayer$ layers gives an effective receptive field that grows linearly with depth.

    \item \textbf{Effective receptive field:} After $L$ layers of windowed attention, position $i$ can be influenced by information from positions as far back as $i - L \cdot W$. This is because each layer propagates information one window-width backward through the residual stream:
    \begin{equation}
        \text{effective receptive field} = \min(i,\; L \cdot W)
    \end{equation}
    At debug scale: $L = 4$ layers $\times$ $W = 128$ = 512 positions, which covers the full sequence. Thus even with windowed attention, the model can in principle propagate information across the entire context through multi-layer composition.

    \item \textbf{Training vs.\ generation asymmetry:} A key design decision is that the window restriction applies only during training. At generation time, the full KV-cache is used without window restriction:
    \begin{center}
    \begin{tabular}{lll}
    \toprule
    Phase & Attention scope & Rationale\\
    \midrule
    Training & $[i - W + 1, \; i]$ & Inductive bias, reduced FLOPs\\
    Generation & $[0, \; i]$ (full) & Maximum expressiveness at inference\\
    \bottomrule
    \end{tabular}
    \end{center}
    This asymmetry means the model is trained with a local inductive bias but evaluated with access to all context. The model learns to write useful information into the residual stream within each window, and at inference time can optionally use longer-range key-value pairs if they happen to be useful.

    \item \textbf{Complexity analysis:}
    \begin{center}
    \begin{tabular}{lcc}
    \toprule
    & Full Causal Attention & Sliding Window ($W < T$)\\
    \midrule
    Time per layer & $O(T^2 \cdot \dhead)$ & $O(T \cdot W \cdot \dhead)$\\
    Effective reduction & --- & factor of $T / W = 4$ in this project\\
    KV-cache (generation) & $O(T \cdot \dhead)$ & $O(T \cdot \dhead)$ (same --- no window at inference)\\
    \bottomrule
    \end{tabular}
    \end{center}
    With the flash\_attn kernel, the window is implemented as a kernel parameter rather than a mask matrix. The kernel simply skips loading tiles outside the window, achieving the computational savings without materializing any mask.

    \item \textbf{Connection to RoPE:} SWA uses RoPE identically to V1. Rotary position embeddings are applied to Q and K before the attention computation. Within the window, relative positions range from $0$ to $W - 1$. RoPE is defined for all integer offsets, so no modification is needed. The key insight is that even though the model only attends within a window during training, the RoPE frequencies encode positions beyond the window — this is what enables the full-context generation to work correctly at inference.

    \item \textbf{Relationship to local attention literature:} SWA belongs to a family of sparse attention patterns:
    \begin{center}
    \begin{tabular}{ll}
    \toprule
    Pattern & Description\\
    \midrule
    Full causal & attend to all past positions\\
    Sliding window (SWA) & attend to $W$ most recent positions\\
    Dilated window & attend to every $d$-th position in a larger span\\
    Global + local & some heads attend globally, others locally\\
    Longformer-style & local window + global tokens\\
    \bottomrule
    \end{tabular}
    \end{center}
    SWA is the simplest member: a contiguous local window with no global tokens. Mistral-7B uses exactly this pattern (window size 4096 with 32k context). The simplicity is intentional for this project — it isolates the effect of local attention bias without confounding it with complex sparse patterns.

    \item \textbf{New trade-off:}
    \begin{enumerate}[nosep]
        \item \textbf{Expressiveness loss in training:} Lower layers cannot directly attend to distant positions during training. If certain tasks require long-range attention in early layers (e.g., matching a bracket opened 500 tokens ago), the model must propagate that information through the residual stream layer by layer.
        \item \textbf{Train/test distribution mismatch:} The model trains with restricted attention but generates with full attention. The attention weights at inference may not be well-calibrated for distant positions because those patterns were never trained.
        \item \textbf{No KV-cache savings at inference:} Unlike some serving-optimized systems that evict old KV entries beyond the window, this implementation keeps the full cache. The benefit is purely in training FLOPs.
        \item \textbf{Minimal implementation complexity:} Because SWA is a kernel parameter rather than a structural change, it adds zero parameters, zero new modules, and zero changes to the cache format. It is the lightest possible attention-pattern experiment.
    \end{enumerate}

    \item \textbf{Parameter count impact:} Zero. The window restriction changes the attention computation but not the model parameters. QKV projections, output projection, RoPE buffers, FFN weights, and normalization scales are all identical to V1. Formally: $|\vtheta_{\text{SWA}}| = |\vtheta_{\text{V1}}|$ at every scale.

    \item \textbf{Connection to linear attention (V5):} SWA and linear attention address the same design pressure ($O(T^2)$ cost) from opposite directions. SWA keeps the softmax kernel but restricts the number of positions attended (reducing arithmetic by a constant factor). Linear attention changes the kernel itself to enable $O(T)$ recurrence but attends to all past positions (with implicit rank-limited weighting). Neither achieves both full-range attention and sub-quadratic complexity simultaneously.

    \item \textbf{Correctness properties verified by tests:}
    \begin{enumerate}[nosep]
        \item Window size validation rejects out-of-range values ($W < 1$ or $W > T$)
        \item Training kernel receives \texttt{window\_size=(W, W)} argument
        \item Generation kernel does \emph{not} receive window\_size (full attention at inference)
        \item Window is computed as $W = \lfloor T / 4 \rfloor$ for all scales
        \item Parameter count is identical to V1 at every scale
        \item Existing variants (V1, V2, V3) produce unchanged outputs after SWA addition
    \end{enumerate}
\end{itemize}

% --- Linear Attention ---
\item \textbf{Linear Attention: ELU-Based Causal Attention (V5)} \emph{(home: \cref{sec:attention-position})}

\begin{itemize}[nosep]
    \item \textbf{Baseline role:} Standard softmax attention computes a weighted average over values using normalized exponential scores:
    \begin{equation}
        \mathrm{Attn}(\mQ,\mK,\mV)_i = \frac{\sum_{j \leq i} \exp(\vq_i^\top \vk_j / \sqrt{\dhead})\, \vv_j}{\sum_{j \leq i} \exp(\vq_i^\top \vk_j / \sqrt{\dhead})}
    \end{equation}
    This requires materializing (or tiling over) the $T \times T$ attention score matrix, giving $O(T^2 \dhead)$ time and $O(T^2)$ or $O(T)$ space depending on implementation strategy.

    \item \textbf{Limitation:} Even with FlashAttention's tiling, the \emph{arithmetic cost} remains $O(T^2 \dhead)$. For very long sequences or research settings where wall-clock scaling matters, quadratic cost is fundamentally limiting. Additionally, the softmax normalization couples all positions — there is no closed-form incremental update rule that avoids revisiting past scores.

    \item \textbf{Design pressure:} Achieve sub-quadratic \emph{time} complexity for causal attention while preserving: (1)~strict causal ordering (no future leakage), (2)~a proper normalization so outputs are bounded, and (3)~compatibility with the existing ModernTransformer shell (same interface, same parameter count). The target is $O(T \cdot \dhead^2)$ time and $O(\dhead^2)$ recurrence space.

    \item \textbf{Replacement mechanism --- kernel linearization:} Replace softmax with a feature map $\varphi$ applied to Q and K separately. The key identity is:
    \begin{equation}
        \mathrm{Attn}(\mQ,\mK,\mV)_i \approx \frac{\sum_{j \leq i} \varphi(\vq_i)^\top \varphi(\vk_j) \cdot \vv_j}{\sum_{j \leq i} \varphi(\vq_i)^\top \varphi(\vk_j)}
    \end{equation}
    Because $\varphi(\vq_i)$ does not depend on $j$, it can be factored out of the sum:
    \begin{equation}\label{eq:linear_attn_factored}
        \vo_i = \frac{\varphi(\vq_i)^\top \mS_i}{\varphi(\vq_i)^\top \vz_i}
    \end{equation}
    where the accumulators are defined recursively:
    \begin{align}
        \mS_i &= \mS_{i-1} + \varphi(\vk_i) \vv_i^\top \in \reals^{\dhead \times \dhead} \label{eq:S_recurrence}\\
        \vz_i &= \vz_{i-1} + \varphi(\vk_i) \in \reals^{\dhead} \label{eq:z_recurrence}
    \end{align}
    with $\mS_0 = \bm{0}$ and $\vz_0 = \bm{0}$. This recurrence is naturally causal (each step depends only on past) and costs $O(\dhead^2)$ per step for the outer product update.

    \item \textbf{Feature map choice --- ELU+1:} The project uses:
    \begin{equation}\label{eq:elu_feature_map}
        \varphi(x) = \mathrm{ELU}(x) + 1 = \begin{cases} x + 1 & x \geq 0 \\ e^x & x < 0 \end{cases}
    \end{equation}
    applied element-wise. This satisfies the critical requirement $\varphi(x) > 0$ for all $x \in \reals$, which guarantees:
    \begin{enumerate}[nosep]
        \item The normalisation denominator $\varphi(\vq_i)^\top \vz_i > 0$ (both vectors have strictly positive entries after at least one key has been accumulated).
        \item The ``attention weights'' (implicit in the factored form) are non-negative, preserving the interpretation of attention as a weighted average.
    \end{enumerate}

    \item \textbf{Mathematical properties of $\varphi$:}
    \begin{itemize}[nosep]
        \item \textbf{Positivity:} $\varphi(x) > 0$ for all $x \in \reals$. For $x \geq 0$: $\varphi(x) = x + 1 \geq 1 > 0$. For $x < 0$: $\varphi(x) = e^x \in (0, 1)$.
        \item \textbf{Range partitioning:} For negative inputs, $\varphi$ maps $(-\infty, 0) \to (0, 1)$. For non-negative inputs, $\varphi$ maps $[0, \infty) \to [1, \infty)$. The function is continuous at $x = 0$ since both branches give $\varphi(0) = 1$.
        \item \textbf{Monotonicity:} $\varphi'(x) = 1$ for $x \geq 0$ and $\varphi'(x) = e^x > 0$ for $x < 0$. Thus $\varphi$ is strictly monotone increasing everywhere.
        \item \textbf{Derivative continuity:} $\varphi'(0^-) = e^0 = 1 = \varphi'(0^+)$. The function is $C^1$ everywhere (smooth gradient flow during backpropagation).
    \end{itemize}
    ...
    \midrule
    Time per layer & $O(T^2 \dhead)$ & $O(T \cdot \dhead^2)$\\
    Space (working) & $O(T^2)$ or $O(T)$ (tiled) & $O(\dhead^2)$ (accumulator)\\
    Output storage & $O(T \cdot \dhead)$ & $O(T \cdot \dhead)$\\
    Crossover regime & favourable when $T < \dhead$ & favourable when $T > \dhead$\\
    \bottomrule
    \end{tabular}
    \end{center}
    At the project's debug scale ($T = 512$, $\dhead = 64$), $T \gg \dhead$, so linear attention is in its favourable regime arithmetically. However, the Python-loop implementation has constant overhead per step that dominates at small $T$.

    \item \textbf{Why no position encoding:} RoPE works because it exploits the \emph{multiplicative structure of softmax}: $\exp(\widetilde{\vq}_m^\top \widetilde{\vk}_n) = \exp(\vq_m^\top \mR_{n-m} \vk_n)$, so the rotation decomposes into a relative offset inside the exponential. In linear attention, the kernel is $\varphi(\vq)^\top \varphi(\vk)$, which is a dot product of two independently transformed vectors. Rotating Q and K before applying $\varphi$ does not produce a clean relative-offset decomposition because $\varphi$ is not multiplicatively compatible with rotations:
    \begin{equation}
        \varphi(\mR_m \vq)^\top \varphi(\mR_n \vk) \neq f(n - m, \vq, \vk) \quad \text{in general}
    \end{equation}
    Position information must therefore come from elsewhere (token embeddings in the residual stream, or future work on position-aware feature maps).

    \item \textbf{Numerical stability --- $\varepsilon$-clamping:} The normalisation denominator can become very small when $\varphi(\vq_i)$ is nearly orthogonal to the accumulated $\vz_i$. To prevent division by zero or numerically unstable outputs:
    \begin{equation}
        \hat{d}_i = \max\left(\varphi(\vq_i)^\top \vz_i,\; \varepsilon\right), \qquad \varepsilon = 10^{-6}
    \end{equation}
    This clamping introduces a small bias at affected positions (outputs are slightly smaller than the true normalised value) but ensures finite gradients everywhere.

    \item \textbf{Causal correctness guarantee:} The recurrence structure in \cref{eq:S_recurrence,eq:z_recurrence} ensures causality by construction. At step $i$, the accumulator $\mS_i$ contains information only from positions $1, \ldots, i$. No future position $j > i$ can influence $\mS_i$ or $\vz_i$ because the loop processes positions left-to-right. This is formally equivalent to a lower-triangular implicit attention matrix where entry $(i, j) = 0$ for $j > i$.

    \item \textbf{Relationship to kernel methods:} Linear attention can be viewed through the lens of kernel approximation. Standard softmax attention with the exponential kernel $\kappa(\vq, \vk) = \exp(\vq^\top \vk / \sqrt{\dhead})$ can be approximated by $\kappa(\vq, \vk) \approx \varphi(\vq)^\top \varphi(\vk)$ for appropriate $\varphi$. The ELU+1 map is not derived from approximating the softmax kernel specifically --- it is a simpler positive map that enables the recurrence factorization. More sophisticated feature maps (random Fourier features, positive orthogonal features) can approximate softmax more closely but add complexity.

    \item \textbf{Connection to state-space models and RNNs:} The accumulator recurrence $\mS_i = \mS_{i-1} + \varphi(\vk_i)\vv_i^\top$ is structurally similar to a linear recurrent neural network with additive state updates. The ``state'' is $\mS \in \reals^{\dhead \times \dhead}$, updated at each step by a rank-1 outer product. This connection places linear attention on the spectrum between:
    \begin{center}
    \begin{tabular}{lll}
    \toprule
    Architecture & State size & Attention pattern\\
    \midrule
    RNN (e.g., LSTM) & $O(\dhead)$ & implicit, via hidden state\\
    Linear Attention & $O(\dhead^2)$ & explicit rank-1 updates\\
    Full Attention & $O(T \cdot \dhead)$ & explicit $T \times T$ matrix\\
    \bottomrule
    \end{tabular}
    \end{center}
    Linear attention trades off between expressiveness (smaller state than full attention) and recurrence efficiency (constant-time update per step, unlike full attention's growing context).

    \item \textbf{New trade-off:}
    \begin{enumerate}[nosep]
        \item \textbf{Expressiveness vs.\ efficiency:} Softmax attention can represent arbitrary attention patterns (any row-stochastic matrix for given Q, K). Linear attention's implicit weights are constrained by the rank of the outer-product accumulator --- it cannot represent patterns that require more than $\dhead$ ``basis'' attention columns.
        \item \textbf{No KV-cache generation:} The recurrence is efficient for training (process all $T$ positions in one pass) but autoregressive generation would require maintaining and querying the $\mS, \vz$ state. This is feasible but not implemented per ADR 0002 (training-comparison-only variant).
        \item \textbf{Python loop overhead:} The sequential recurrence requires $T$ iterations. Without a fused CUDA kernel or \texttt{torch.compiler} scan primitive, the Python loop adds significant per-step overhead. At $T = 512$, this makes V5 slower in wall-clock than FlashAttention-based V1, despite lower arithmetic complexity.
        \item \textbf{No learned position:} Without RoPE or ALiBi, the model relies entirely on token order information propagated through the residual stream across layers. Early layers have no explicit position signal; position awareness emerges only through the sequential processing pattern.
    \end{enumerate}

    \item \textbf{Parameter count impact:} Linear attention uses the same $4\dmodel^2$ parameters per layer as standard MHA (three QKV projections + one output projection, all of shape $\dmodel \times \dmodel$). No RoPE buffers (non-learnable). The total parameter count is within $\pm 0\%$ of V1, well within the $\pm 5\%$ parity requirement for fair comparisons.

    \item \textbf{Correctness properties verified by property-based tests:}
    \begin{enumerate}[nosep]
        \item $\varphi(x) > 0$ for all $x \in [-100, 100]$ (strict positivity of feature map)
        \item For $x < 0$: $\varphi(x) \in (0, 1)$ (range characterization)
        \item Causal independence: modifying input at position $t$ does not change output at positions $< t$
        \item Finite outputs for all inputs in $[-10, 10]$ (numerical stability)
        \item Output shape $(B, T, \dmodel)$ and second return element is \texttt{None} (interface contract)
    \end{enumerate}
\end{itemize}
...
% --- V4-interleaved ---
\item \textbf{Interleaved SWA: Alternating Local and Global Layers (V4-interleaved)} \emph{(home: \cref{sec:attention-position})}

\begin{itemize}[nosep]
    \item \textbf{Baseline role:} V4 (SWA) applies the same window restriction at every layer. All $\nlayer$ layers attend to only the $W$ most recent tokens. Long-range information can only propagate through multi-layer residual composition.
    \item \textbf{Limitation:} Pure SWA relies entirely on the residual stream to relay long-range information layer by layer. If the model needs direct access to a distant token (e.g., matching a parenthesis opened far away), it must compress that information into the residual stream within the window at an earlier layer. This places a strong representational burden on lower layers and may degrade tasks requiring long-range retrieval.
    \item \textbf{Design pressure:} Isolate the question: ``Does periodic full-context access recover the information lost by windowing?'' If alternating one full-attention layer between every SWA layer recovers most of the quality loss, it suggests that only a few global layers are needed and the remaining layers can be efficient local layers.
    \item \textbf{Replacement mechanism:} Even-indexed layers ($\ell = 0, 2, 4, \ldots$) use full causal attention (no window restriction). Odd-indexed layers ($\ell = 1, 3, 5, \ldots$) use sliding window attention with $W = \lfloor T / 4 \rfloor$. Each layer uses the same \texttt{FlashAttention} module but with per-layer \texttt{window\_size} configuration:
    \begin{equation}
        W^{(\ell)} = \begin{cases}
            \text{None (full attention)}, & \ell \mod 2 = 0,\\
            \lfloor T/4 \rfloor, & \ell \mod 2 = 1.
        \end{cases}
    \end{equation}
    This is inspired by Gemma-2-style layer interleaving. The registry constructs per-layer configs via a \texttt{per\_layer\_config\_builder} function.
    \item \textbf{Complexity:} For $L$ layers with half global and half local:
    \begin{equation}
        \text{Time} = \frac{L}{2} \cdot O(T^2 \dhead) + \frac{L}{2} \cdot O(T \cdot W \cdot \dhead) = O\!\left(\frac{L}{2}(T^2 + TW)\dhead\right)
    \end{equation}
    Compared to full attention at every layer ($O(LT^2\dhead)$), this saves roughly $\frac{1}{2}(1 - W/T) = \frac{3}{8}$ of the attention FLOPs.
    \item \textbf{New trade-off:} Gains some long-range access (every other layer is global) at the cost of recovering half of the full-attention compute. The experiment tests whether this 50/50 split is better than pure SWA or pure full attention on the same token budget. Additionally, the per-layer config adds implementation complexity (each block may have a different effective attention pattern).
    \item \textbf{Parameter count impact:} Zero. All layers use the same projections. The only difference is which layers pass \texttt{window\_size} to the attention kernel.
    \item \textbf{Connection to V4 (pure SWA):} V4-interleaved is a sub-variant of V4. If the interleaved variant matches V1's quality while V4 degrades, it demonstrates that the SWA quality loss is due to insufficient long-range access rather than some other artifact of the window mechanism.
\end{itemize}

% --- Evaluation Comparison Axes ---
\item \textbf{Three-Axis Comparison Framework} \emph{(home: \cref{sec:training-eval})}

\begin{itemize}[nosep]
    \item \textbf{Baseline role:} Comparing Transformer variants on final validation loss alone conflates quality improvements with compute-efficiency improvements. A variant with more FLOPs per step may reach better loss simply because it processes data differently, not because its architecture is better at a fixed cost.
    \item \textbf{Limitation:} A single metric (e.g., val loss at fixed training steps) cannot distinguish whether a variant is better because it learns faster per token, because it processes tokens faster in wall-clock time, or because it extracts more learning from each FLOP.
    \item \textbf{Design pressure:} Need simultaneous controlled perspectives that separate quality, time-efficiency, and compute-efficiency. All comparisons should use the same training runs (no additional experiments needed).
    \item \textbf{Replacement mechanism --- three comparison axes applied post-hoc:}
    \begin{center}
    \begin{tabular}{lll}
    \toprule
    Axis & Control variable & Metric\\
    \midrule
    Fixed-data & Same token budget $B_{\text{tok}}$ & Val loss $\loss(B_{\text{tok}})$\\
    Fixed-wallclock & Same elapsed time $t_{\max}$ & Val loss $\loss(t_{\max})$\\
    Fixed-FLOPs & Same cumulative FLOPs $F_{\max}$ & Val loss $\loss(F_{\max})$\\
    \bottomrule
    \end{tabular}
    \end{center}
    The wall-clock and FLOP budgets are determined dynamically as $t_{\max} = \min_v t_v^{\text{total}}$ and $F_{\max} = \min_v F_v^{\text{total}}$ respectively. Loss values at intermediate budgets are interpolated from training logs.
    \item \textbf{New trade-off:} Fixed-data comparisons favour architectures that learn more per token but may be slower. Fixed-wallclock comparisons favour architectures that are fast even if they learn less per token. Fixed-FLOPs comparisons favour architectures with lower per-step FLOP cost. Reporting all three prevents cherry-picking one axis that favours a particular variant.
    \item \textbf{Mathematical formulation:} Given training logs $\{(s, \loss_s, t_s, F_s)\}$ where $s$ is the step, $t_s$ is cumulative wall-clock, and $F_s$ is cumulative FLOPs, define:
    \begin{align}
        \loss^{\text{data}}(B) &= \text{interpolate}(\{(\text{tokens}_s, \loss_s)\}, B)\\
        \loss^{\text{time}}(\tau) &= \text{interpolate}(\{(t_s, \loss_s)\}, \tau)\\
        \loss^{\text{flop}}(F) &= \text{interpolate}(\{(F_s, \loss_s)\}, F)
    \end{align}
\end{itemize}

% --- Component-Level FLOP Accounting ---
\item \textbf{Component-Level FLOP Accounting and MFU} \emph{(home: \cref{sec:training-eval})}

\begin{itemize}[nosep]
    \item \textbf{Baseline:} The common approximation $6NT$ (where $N$ is parameter count, $T$ is tokens per step) gives a rough FLOP estimate but does not distinguish between variants with different attention costs. V4 (SWA) does less attention work than V1 for the same $N$ and $T$; the $6NT$ approximation cannot capture this.
    \item \textbf{Design pressure:} Need per-step FLOP estimates that reflect actual computation at the component level, so fixed-FLOP comparisons are meaningful across variants with different attention mechanisms.
    \item \textbf{Mechanism:} Sum FLOPs from each component class per layer:
    \begin{align}
        F_{\text{QKV}} &= 3 \cdot \nlayer \cdot 2 \cdot T \cdot \dmodel^2 \quad \text{(Q, K, V projections)}\\
        F_{\text{attn}} &= \nlayer \cdot 2 \cdot T \cdot C_{\text{attn}} \cdot \dhead \cdot \nhead \quad \text{(attention scores)}\\
        F_{\text{out}} &= \nlayer \cdot 2 \cdot T \cdot \dmodel^2 \quad \text{(output projection)}\\
        F_{\text{FFN}} &= \nlayer \cdot 2 \cdot n_{\text{proj}} \cdot T \cdot \dmodel \cdot d_h \quad \text{(FFN projections)}
    \end{align}
    where $C_{\text{attn}}$ is the effective context length (variant-dependent):
    \begin{equation}
        C_{\text{attn}} = \begin{cases}
            T & \text{full attention (V0, V1, V3)}\\
            W & \text{SWA (V4)}\\
            r & \text{Linformer (V5), projection rank}
        \end{cases}
    \end{equation}
    A $3\times$ training multiplier accounts for forward + backward passes.
    \item \textbf{Model FLOPs Utilization (MFU):} The ratio of achieved computation rate to hardware peak:
    \begin{equation}
        \mathrm{MFU} = \frac{F_{\text{step}} / t_{\text{step}}}{F_{\text{peak}}}
    \end{equation}
    where $F_{\text{peak}} = 242$ TFLOPS (L4 BF16 peak). MFU distinguishes compute-bound regimes (high MFU, close to hardware peak) from memory-bandwidth-bound regimes (low MFU, limited by data movement).
    \item \textbf{Arithmetic intensity:} The ratio of FLOPs to bytes transferred:
    \begin{equation}
        I = \frac{F_{\text{step}}}{B_{\text{bytes}}}
    \end{equation}
    where $B_{\text{bytes}}$ estimates parameter reads plus activation traffic in BF16. The L4 ridge point is $I^* \approx 807$ FLOPs/byte. Variants with $I < I^*$ are memory-bound; variants with $I > I^*$ are compute-bound.
\end{itemize}

% --- Roofline Diagram ---
\item \textbf{Roofline Diagram} \emph{(home: \cref{sec:training-eval})}

\begin{itemize}[nosep]
    \item \textbf{What it is:} A plot of achieved performance (TFLOPS/s) versus arithmetic intensity (FLOPs/byte). The hardware ceiling creates a characteristic ``roofline'' shape:
    \begin{equation}
        P_{\text{achievable}}(I) = \min\!\left(F_{\text{peak}},\; \mathrm{BW} \cdot I\right)
    \end{equation}
    where $\mathrm{BW} = 300$ GB/s (L4 memory bandwidth) and $F_{\text{peak}} = 242$ TFLOPS.
    \item \textbf{Use in this project:} Each variant is plotted as a point on the roofline at its measured $(I, P)$ coordinates. Variants that sit on the diagonal (bandwidth-limited regime) benefit from reduced memory traffic. Variants on the flat ceiling (compute-limited regime) benefit from reduced arithmetic. This tells us whether a variant's wall-clock advantage comes from doing less math or from being more cache-friendly.
    \item \textbf{Ridge point:} The crossover occurs at $I^* = F_{\text{peak}} / \mathrm{BW} = 242 \times 10^{12} / (300 \times 10^9) \approx 807$ FLOPs/byte.
\end{itemize}

% --- Stable Rank ---
\item \textbf{Stable Rank: Representation Health Diagnostic} \emph{(home: \cref{sec:training-eval})}

\begin{itemize}[nosep]
    \item \textbf{Definition:} For a hidden-state matrix $\mH \in \reals^{n \times d}$ (where $n$ is the number of tokens and $d = \dmodel$), the stable rank is:
    \begin{equation}
        \mathrm{srank}(\mH) = \frac{\|\mH\|_F^2}{\|\mH\|_2^2} = \frac{\sum_i \sigma_i^2}{\sigma_1^2}
    \end{equation}
    where $\sigma_1 \geq \sigma_2 \geq \ldots$ are singular values. The stable rank is bounded: $1 \leq \mathrm{srank}(\mH) \leq \min(n, d)$.
    \item \textbf{Interpretation:} Stable rank measures the effective dimensionality of the representation. If $\mathrm{srank}(\mH) = 1$, all tokens lie on a single direction (representation collapse). If $\mathrm{srank}(\mH) \approx d$, the representation uses the full available space. A healthy model should maintain reasonable stable rank throughout its layers; a sharp drop in deep layers suggests the model is over-compressing information.
    \item \textbf{Measurement protocol:} Computed at the final checkpoint, averaged over $\sim 100$ validation batches. Reported per-layer as a curve: $\mathrm{srank}^{(\ell)}$ for $\ell = 0, \ldots, \nlayer - 1$.
    \item \textbf{Why it matters for variant comparison:} Different attention patterns (full, windowed, linear) create different representational dynamics. A variant that aggressively compresses context (e.g., V5 linear attention with its finite-rank accumulator) might show earlier stable-rank collapse than full attention. This provides evidence about whether a variant's loss difference is due to representational capacity limits or other factors.
\end{itemize}

% --- CKA ---
\item \textbf{Centered Kernel Alignment (CKA)} \emph{(home: \cref{sec:training-eval})}

\begin{itemize}[nosep]
    \item \textbf{Definition:} For representations $\mX \in \reals^{n \times p}$ and $\mY \in \reals^{n \times q}$ from two layers (same inputs), linear CKA is:
    \begin{equation}
        \mathrm{CKA}(\mX, \mY) = \frac{\|\mY^\top \mX\|_F^2}{\|\mX^\top \mX\|_F \cdot \|\mY^\top \mY\|_F}
    \end{equation}
    (after centering both matrices). CKA $\in [0, 1]$, with $\mathrm{CKA}(\mX, \mX) = 1$.
    \item \textbf{Properties:}
    \begin{itemize}[nosep]
        \item Invariant to orthogonal transformations and isotropic scaling
        \item Symmetric: $\mathrm{CKA}(\mX, \mY) = \mathrm{CKA}(\mY, \mX)$
        \item Self-similarity: diagonal entries of the full $L \times L$ matrix equal 1
    \end{itemize}
    \item \textbf{Use:} High CKA between non-adjacent layers $(\ell, \ell + k)$ for $k > 1$ indicates that those layers perform redundant computation --- the representation did not change much. The primary presentation is an adjacent-layer curve $\mathrm{CKA}(\mX^{(\ell)}, \mX^{(\ell+1)})$ for $\ell = 0, \ldots, \nlayer - 2$. The full $L \times L$ heatmap is supplementary.
    \item \textbf{Interpretation for variants:} If V4 (SWA) shows high adjacent-CKA in windowed layers but low CKA across global layers in V4-interleaved, it suggests that global layers introduce genuinely new information while windowed layers are more incremental.
\end{itemize}

% --- MQAR ---
\item \textbf{Multi-Query Associative Recall (MQAR)} \emph{(home: \cref{sec:training-eval})}

\begin{itemize}[nosep]
    \item \textbf{What it tests:} Whether the model can bind key-value token pairs planted at known positions in a synthetic sequence and recall the correct value when prompted with the key later.
    \item \textbf{Protocol:} Generate sequences containing $k$ key-value associations $(a_1 \to b_1, \ldots, a_k \to b_k)$ at controlled distances. After all pairs, present query tokens (the keys) and measure $P(\text{correct value} \mid \text{key query})$.
    \item \textbf{Metric:} Overall accuracy and accuracy-by-distance (how recall degrades as the retrieval distance increases).
    \item \textbf{Why it matters:} This probe isolates retrieval capacity. Variants with restricted attention (V4-SWA, V5-Linformer) should show measurable degradation at distances beyond their effective receptive field, while full-attention variants (V1, V2, V3) should be roughly constant. It provides a mechanistic explanation for any loss differences observed in the three-axis comparison.
    \item \textbf{Mathematical formulation:} Let $d_j$ be the distance (in tokens) between the key-value binding and the query for association $j$. The per-distance accuracy is:
    \begin{equation}
        \text{acc}(d) = \frac{1}{|\{j : d_j = d\}|} \sum_{j : d_j = d} \ind[\argmax_w p_\vtheta(w \mid \text{context}_j) = b_j]
    \end{equation}
\end{itemize}

% --- ICL Loss Decay ---
\item \textbf{ICL Loss Decay Exponent ($\alpha$)} \emph{(home: \cref{sec:training-eval})}

\begin{itemize}[nosep]
    \item \textbf{Definition:} The per-position validation loss $L(t)$ typically decays as a power law with sequence position $t$:
    \begin{equation}
        L(t) = A \cdot t^{-\alpha} + C
    \end{equation}
    where $A > 0$ is an amplitude, $\alpha > 0$ is the decay exponent, and $C \geq 0$ is the irreducible loss floor.
    \item \textbf{Interpretation:} A steep $\alpha$ (large negative power) means the model effectively uses context --- each additional token of context significantly reduces prediction uncertainty. A small $\alpha$ indicates the model hits diminishing returns from context early. $C$ represents the information-theoretic limit that cannot be reduced by additional context.
    \item \textbf{Fitting procedure:} Non-linear least squares (\texttt{scipy.optimize.curve\_fit}) on the per-position loss curve measured over validation data. Report $\alpha$, $C$, and goodness-of-fit $R^2$.
    \item \textbf{Variant comparison:} V4 (SWA) may show a flatter $\alpha$ because window-restricted layers cannot directly leverage distant context. V5 (linear attention) may show early flattening due to the finite-rank accumulator losing information about distant positions.
\end{itemize}

% --- Attention Entropy ---
\item \textbf{Attention Entropy} \emph{(home: \cref{sec:attention-position})}

\begin{itemize}[nosep]
    \item \textbf{Definition:} For attention weights $\bm{\alpha}_i \in \Delta^T$ (probability simplex) at query position $i$, head $h$, layer $\ell$:
    \begin{equation}
        H^{(\ell,h)}_i = -\sum_{j=1}^{i} \alpha_{ij} \log \alpha_{ij}
    \end{equation}
    Averaged over positions and batches to give per-layer and per-head entropy statistics.
    \item \textbf{Interpretation:} High entropy means the head attends broadly (uniform-like). Low entropy means the head focuses sharply on few positions. Entropy $= 0$ means the head attends to exactly one position. Maximum entropy $= \log(i)$ at position $i$ means uniform attention.
    \item \textbf{Applicability:} Only computable for variants where attention weights are materialized (V0, V5). Flash-attention-based variants (V1--V4) do not produce the explicit attention matrix, so attention entropy returns None for these variants. This is a known limitation, not a bug.
    \item \textbf{Use:} Identifies degenerate attention patterns (all entropy near zero suggests the model collapses to trivial routing; all entropy near maximum suggests the model fails to discriminate).
\end{itemize}

% --- Pareto Front Analysis ---
\item \textbf{Pareto Front Analysis} \emph{(home: \cref{sec:training-eval})}

\begin{itemize}[nosep]
    \item \textbf{Definition:} A variant $v$ is Pareto-optimal on the $(x, y)$ objective pair if no other variant achieves both lower $x$ and lower $y$. Formally, $v$ is non-dominated if:
    \begin{equation}
        \nexists\, v' \neq v : x_{v'} \leq x_v \;\wedge\; y_{v'} \leq y_v \;\wedge\; (x_{v'} < x_v \;\vee\; y_{v'} < y_v)
    \end{equation}
    \item \textbf{Objective pairs reported:}
    \begin{center}
    \begin{tabular}{lll}
    \toprule
    $x$ (cost) & $y$ (quality) & What it reveals\\
    \midrule
    Cumulative FLOPs & Val loss & Compute-efficiency frontier\\
    Wall-clock time & Val loss & Time-efficiency frontier\\
    Peak GPU memory & Val loss & Memory-efficiency frontier\\
    \bottomrule
    \end{tabular}
    \end{center}
    \item \textbf{Interpretation:} Variants on the Pareto front represent the best achievable trade-offs --- no other variant dominates them on both axes. A variant behind the front is strictly inferior: another variant exists that is both cheaper and better (or equal on one axis and strictly better on the other).
\end{itemize}

% --- Seed Aggregation ---
\item \textbf{Statistical Reporting: Seed Aggregation} \emph{(home: \cref{sec:training-eval})}

\begin{itemize}[nosep]
    \item \textbf{Protocol:} All results are reported as mean $\pm$ standard deviation over 3+ random seeds. Let $\{m_1, \ldots, m_S\}$ be metric values from $S$ seeds:
    \begin{equation}
        \bar{m} = \frac{1}{S}\sum_{s=1}^S m_s, \qquad \sigma_m = \sqrt{\frac{1}{S-1}\sum_{s=1}^S (m_s - \bar{m})^2}
    \end{equation}
    \item \textbf{Reporting format:} ``$\bar{m} \pm \sigma_m$'' in all comparison tables and visualizations.
    \item \textbf{Confidence threshold:} If $S < 3$, results are flagged as lacking confidence and additional seeds may be added selectively for variant pairs with overlapping intervals.
    \item \textbf{Why seeds matter:} Training involves stochastic elements (initialization, data order, dropout if used). Reporting single-seed results can lead to false conclusions --- one variant may appear better due to a lucky seed. Multiple seeds give statistical evidence of true architectural advantages.
\end{itemize}

% --- Parameter Parity Validation ---
\item \textbf{Parameter Parity Validation} \emph{(home: \cref{sec:training-eval})}

\begin{itemize}[nosep]
    \item \textbf{Rule:} All compared variants must have parameter counts within $\pm 5\%$ of their mean. If variant $v$ has $N_v$ trainable parameters:
    \begin{equation}
        \frac{|N_v - \bar{N}|}{\bar{N}} \leq 0.05 \quad \forall v, \qquad \bar{N} = \frac{1}{|V|}\sum_{v \in V} N_v
    \end{equation}
    \item \textbf{Exception:} Variants whose mechanism \emph{inherently} reduces parameters (e.g., GQA removes KV projection parameters) may exceed this tolerance. The difference is noted in results rather than compensated (e.g., by widening the FFN).
    \item \textbf{Purpose:} Ensures that quality differences are attributable to architectural mechanisms rather than raw capacity differences. A model with 20\% more parameters might win simply due to more capacity, not a better inductive bias.
\end{itemize}

% --- ProbeTarget Protocol ---
\item \textbf{ProbeTarget Protocol (Software Architecture)} \emph{(home: \cref{sec:infrastructure})}

\begin{itemize}[nosep]
    \item \textbf{What it is:} A structural protocol that decouples evaluation probes from model internals. Probes program against \texttt{ProbeTarget} rather than reaching into \texttt{model.blocks} or registering hooks directly.
    \item \textbf{Two methods:}
    \begin{enumerate}[nosep]
        \item \texttt{forward(x) $\to$ logits} --- lightweight, sufficient for MQAR (only needs output logits)
        \item \texttt{forward\_with\_internals(x) $\to$ ProbeInternals} --- returns per-layer hidden states for stable rank, CKA, and entropy probes
    \end{enumerate}
    \item \textbf{Design rationale:} Separating these two methods avoids OOM on large models when only logits are needed. The heavy path (capturing all layer outputs) is opt-in.
    \item \textbf{Adapter pattern:} \texttt{ModelProbeAdapter} wraps real models (V0--V5) to satisfy the \texttt{ProbeTarget} protocol. Probes never import model-specific classes.
    \item \textbf{Connection to testing:} Test fakes satisfy \texttt{ProbeTarget} without inheritance (structural typing via Python's \texttt{Protocol}). This allows evaluation tests to run without GPU or real model weights.
\end{itemize}

% --- AttentionModule Protocol ---
\item \textbf{AttentionModule Protocol (Software Architecture)} \emph{(home: \cref{sec:attention-position})}

\begin{itemize}[nosep]
    \item \textbf{What it is:} A structural protocol formalizing the contract that all attention variants (V0 CausalSelfAttention, V1 ModernAttention, V2 ALiBiAttention, V3 GQAAttention, V4 FlashAttention with window, V5 LinearAttention) already satisfy informally:
    \begin{equation}
        \texttt{forward}(\vx, \texttt{kv\_cache}) \to (\texttt{output}, \texttt{new\_kv\_cache})
    \end{equation}
    \item \textbf{Purpose:} Documentary, not enforcement. Makes the shared interface discoverable without requiring all variants to inherit from a common base class. Type annotations in \texttt{TransformerBlock} and \texttt{ModernTransformerBlock} use this protocol.
    \item \textbf{Cache type:} Deliberately uses \texttt{Any} for the cache type because V0/V1 (concat-based 2-tuple) and V1--V4 (pre-allocated 3-tuple via flash\_attn) use incompatible cache shapes. Unifying cache types is deferred.
    \item \textbf{Connection to GQA:} GQA's cache shape is $(B, n_{\mathrm{kv}}, T, \dhead)$ rather than $(B, \nhead, T, \dhead)$. The protocol's \texttt{Any} cache type accommodates this without specialization.
\end{itemize}

% --- Evaluation Pipeline ---
\item \textbf{Evaluation Pipeline (Software Architecture)} \emph{(home: \cref{sec:infrastructure})}

\begin{itemize}[nosep]
    \item \textbf{What it is:} A single-interface module that orchestrates the full post-hoc evaluation workflow:
    \begin{equation}
        \text{checkpoints} \xrightarrow{\text{pipeline}} \text{ReportResult (structured data + files)}
    \end{equation}
    The pipeline loads checkpoints, computes FLOPs, runs probes, slices comparisons across three axes, aggregates seeds, generates visualizations, and writes reports.
    \item \textbf{Interface:} \texttt{EvaluationPipeline.run(checkpoints, output\_dir) $\to$ ReportResult}. Callers (CLI, tests, notebooks, future dashboard) share this one entry point.
    \item \textbf{Design decisions:}
    \begin{itemize}[nosep]
        \item Class (not function) because configuration is captured once (device, data\_dir) and methods are extensible
        \item Returns \texttt{ReportResult} alongside file writes so tests and dashboards get structured data without re-parsing files
        \item Lenient error handling: pipeline always produces a partial report with a warnings list; callers inspect warnings to decide severity
    \end{itemize}
    \item \textbf{Output structure:} A self-contained report directory:
    \begin{center}
    \begin{tabular}{ll}
    \toprule
    Path & Contents\\
    \midrule
    \texttt{plots/*.png} & All visualizations (10 plot types)\\
    \texttt{raw/metrics.csv} & Tabular computed data\\
    \texttt{raw/metrics.json} & Structured computed data\\
    \texttt{summary.md} & Human-readable report with embedded image links\\
    \texttt{metadata.json} & Timestamp, software versions, hardware ID, evaluated checkpoints\\
    \bottomrule
    \end{tabular}
    \end{center}
\end{itemize}
