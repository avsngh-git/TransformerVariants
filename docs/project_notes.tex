...
The recurrence is causal by construction because $(\mS_i,\vz_i)$ contains only indices $j\leq i$. Therefore the model is order-sensitive even without an explicit positional embedding: changing token order changes every subsequent prefix state. However, causality alone does not provide a rich representation of relative distance. Applying RoPE before a nonlinear $\varphi$ remains computationally possible, but the clean rotation identity in \cref{eq:rope_relative} is generally lost after the nonlinear feature map. The project's no-position version should therefore be interpreted as an architectural ablation, not a theorem that linear attention never needs position information.
