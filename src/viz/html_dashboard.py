...
<div class="grid"><div class="card wide"><div class="provenance-grid" id="provenanceCards"></div><div class="provenance-details" id="provenanceDetails"></div></div><div class="card side" id="parity"></div></div></section>
...
const softwareVersions=Object.entries(meta.software_versions||{});
const evaluatedCheckpoints=meta.evaluated_checkpoints||[];
const provenanceItems=[
['Evaluation hardware',meta.hardware||benchmarks.hardware||'Not recorded'],
['Report generated',meta.timestamp||benchmarks.generated_at||'Not recorded'],
['Schema version','v'+(data.schema_version||1)],
['Evaluated checkpoints',evaluatedCheckpoints.length]
];
document.getElementById('provenanceCards').innerHTML=provenanceItems.map(item=>`<div class="provenance-card"><span>${item[0]}</span><strong>${item[1]}</strong></div>`).join('');
const checkpointDetails=evaluatedCheckpoints.length?`<details><summary>Evaluated checkpoints (${evaluatedCheckpoints.length})</summary><ul>${evaluatedCheckpoints.map(checkpoint=>`<li>${checkpoint}</li>`).join('')}</ul></details>`:'';
const softwareDetails=softwareVersions.length?`<details><summary>Software versions (${softwareVersions.length})</summary><ul>${softwareVersions.map(([name,version])=>`<li><b>${name}</b>: ${version}</li>`).join('')}</ul></details>`:'';
const warnings=meta.warnings||[];
const warningDetails=warnings.length?`<details><summary>Methodology warnings (${warnings.length})</summary><ul>${warnings.map(warning=>`<li>${warning}</li>`).join('')}</ul></details>`:'';
...
def _plot_summary(stem: str) -> str:
    """Return the reader-facing interpretation shown below a publication plot."""
...
def _embedded_plots(plots_dir: Path) -> list[dict[str, str]]:
    ...
    for path in paths:
        _, group_title, group_summary = _PLOT_GROUPS[_plot_group_key(path.stem)]
        encoded = base64.b64encode(path.read_bytes()).decode("ascii")
        plots.append(
            {
                "title": path.stem.replace("_", " ").title(),
                "summary": _plot_summary(path.stem),
                "group": group_title,
